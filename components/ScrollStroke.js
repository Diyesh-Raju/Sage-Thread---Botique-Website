...
/* Scroll-drawn stroke (Skiper19), ported to JS + plain CSS.
   Straight port of the original's structure: nothing pins. The statement and
   the artwork sit in one block that scrolls away with the page, and the line
   draws itself as the section is scrolled through — so the reader follows the
   ink down while the words leave at the top. The artwork hangs off that block
   at top / -40% right, which is what starts the ink at the top of the words.
...
/* the line arrives half drawn, so the knot is already there to read */
const START = 0.5;
...
export default function ScrollStroke() {
  ...
  const { scrollYProgress } = useScroll({ target: frame });
  ...
  /* The still state has to name its value rather than drop the style prop.
     ...
     onto the path. Handing back `undefined` stops it updating them but clears
     nothing, which froze the line at START. */

  return (
    <section className="sstroke" ref={frame} aria-label="Sage Thread — on lasting pieces">
      <div className="sstroke__block">
        <h2 className="sstroke__line">
          Beauty fades, but a <br /> perfect piece lasts <br /> forever
        </h2>
